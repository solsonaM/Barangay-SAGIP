...
## Running both servers together (for demos/defense)

Two terminals:

```bash
# Terminal 1
cd ml-service && source venv/bin/activate && uvicorn main:app --port 8001

# Terminal 2
cd barangay-sagip-web && php artisan serve
```

If the ML service is down, request submission still works — `MLClassificationService`
fails soft and routes the request straight to `needs_review` (Feature 5) so
nothing is lost; officials just validate it manually.

---

## What's a working prototype vs. what needs more work before defense

**Solid and tested:**
- All 12 features have real, functioning code paths (not stubs)
- The ML service is genuinely trained and its accuracy is honestly reported
- The classification → validation → auto-assignment pipeline is fully wired end-to-end

**Worth strengthening before your panel:**
- Replace the templated synthetic dataset with real (anonymized) barangay
  incident data as soon as it's available — this is the single highest-leverage
  improvement to the whole system's credibility
- The response-assignment "model" (Feature 6) is currently a transparent,
  weighted scoring function, not a trained model — this matches what the
  proposal names as the baseline approach (Section 9.3), but if your adviser
  wants a learned ranking model, that's a good next iteration
- No automated test suite yet (PHPUnit for Laravel, pytest for the ML
  service) — worth adding before you rely on this for a live demo
- Styling uses Tailwind via CDN for simplicity; consider a proper Vite build
  if you want production-grade asset optimization
